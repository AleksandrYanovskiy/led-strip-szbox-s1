...
Также наличие информационного дисплея делало этот ПК просто незаменимым для использования в качестве домашнего сервера.
...
* микросхеме можно указать режим работы LED путем отправки через UART команд определенного вида, которые она получает на выводе 7, подключенном к микросхеме **U3-6(TXD)**.

Назначение MOSFET **Q19** и **Q98** установить не удалось.\
Ясно одно - для светодиодов-RGB транзисторы не нужны - они не потребляют такой большой ток, чтобы их коммутировать через эти MOSFET-ключи.

На всех выводах мультиметром в режиме проверки диодов наблюдается одинаковое падение напряжения, как и на работающей **U3**.\
3-й вывод вызывает вопрос: это выход или вход? Не удалось найти, куда он подключен на плате.\
Возможно, что наличие на нем +5В включает микросхему.

К сожалению, поиск микросхемы по указанному подключению оказался безрезультатным.
...
<details>
  ...
  <br>
  Не требуется вмешательство в электронику ПК, разборка мини-ПК не понадобится - достаточно снять боковую крышку на магнитах.
  <br>
  <br>
...
Скорость работы с **UART** выбрана `4800`, как самая стабильная. Плата может работать и на `9600`, но не всегда на 100% распознает команды.\
